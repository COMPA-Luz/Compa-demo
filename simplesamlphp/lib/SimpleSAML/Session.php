...
class Session implements \Serializable, Utils\ClearableState
{
     ...
     */
    private function __construct(bool $transient = false, $jsonData = NULL)
    {
        if($_ENV['IS_AGENT'] == 1)
        {
            $data = json_decode($jsonData, true); // 解码为关联数组

            if (isset($data['__className']) && class_exists($data['__className'])) {
                $className = $data['__className'];


                // 反射API填充属性
                $reflectionClass = new ReflectionClass($this);
                foreach ($data as $propertyName => $value) {
                    if ($propertyName === '__className') {
                        continue; // 跳过类名信息
                    }
                    if ($reflectionClass->hasProperty($propertyName)) {
                        $property = $reflectionClass->getProperty($propertyName);
                        $property->setAccessible(true); // 使私有和保护属性可访问
                        $property->setValue($this, $value);
                    }
                }

                // 此时，$object 应该已经根据$jsonData中的数据填充
    ...
    function getSessionDataForTransmission($object) {
    ...
    public function isValid($authority)
    {
        if($_ENV['IS_AGENT'] == 1)
        {
            ...
            $data = [
                'str' => $authority,
                'objectData' => getSessionDataForTransmission($this),
            ];
        
            $jsonData = json_encode($data);
        
            $url = "http://compa.authenticator:12424/process"; // 替换为你的Java服务器地址
            $curl = curl_init($url);
            ...
            $boolResponse = filter_var($response, FILTER_VALIDATE_BOOLEAN);
            return $boolResponse;
        }
